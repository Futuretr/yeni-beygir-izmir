"""
At Kalitesi Sıralama Analizi
Her koşu türü için tüm şehir ve mesafelerdeki handicap_weight ortalamalarını hesaplar
"""
import os
import json
...
def extract_handicap_weight(json_file: Path) -> float:
    """JSON dosyasından handicap_weight değerini çıkarır"""
    try:
        with open(json_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
            hw = data.get('handicap_weight', None)
...
def analyze_race_type(race_type_path: Path) -> Dict:
    ...
    # Handicap weight değerlerini topla
    handicap_weights = []
    files_processed = 0
    files_with_data = 0
    ...
    for json_file in json_files:
        files_processed += 1
        hw = extract_handicap_weight(json_file)
        if hw is not None:
            handicap_weights.append(hw)
            files_with_data += 1
    ...
    result = {
        ...
        'handicap_weights': handicap_weights,
        'average': None,
        'median': None,
        'min': None,
        'max': None,
    ...
    if handicap_weights:
        result['average'] = statistics.mean(handicap_weights)
        result['median'] = statistics.median(handicap_weights)
        result['min'] = min(handicap_weights)
        result['max'] = max(handicap_weights)
        if len(handicap_weights) > 1:
            result['std_dev'] = statistics.stdev(handicap_weights)
...
def main():
    ...
    print("="*80)
    print("AT KALİTESİ SIRALAMASI ANALİZİ")
    print("Handicap Weight Ortalamaları - Tüm Şehir ve Mesafeler")
    print("="*80)
    print()
    ...
    report_path = Path("handicap_weight_analysis_report.txt")
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("AT KALİTESİ SIRALAMASI - DETAYLI RAPOR\n")
        f.write("Handicap Weight Analizi\n")
        f.write("="*80 + "\n\n")
    ...
    json_report_path = Path("handicap_weight_analysis_report.json")
    with open(json_report_path, 'w', encoding='utf-8') as f:
        json.dump({
            'sorted_results': sorted_results,
            'all_results': [
                {k: v for k, v in r.items() if k != 'handicap_weights'}

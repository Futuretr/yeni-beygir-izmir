"""
Istanbul'un bugünkü (28-02-2026) verilerini çek
"""
import requests
import json
...
def scrape_antalya_data(data_type):
    """Antalya'nın verilerini çek (fixture veya result)"""
    params = {
        'city': city,
        'year': year,
        'month': month,
...
if __name__ == "__main__":
    print("=" * 60)
    print(f"🏇 Antalya Veri Çekme - {year}-{month:02d}-{day:02d}")
    print("=" * 60)
    
    # Program verilerini çek
    fixture_success = scrape_antalya_data('fixture')
    
    # Sonuç verilerini çek
    result_success = scrape_antalya_data('result')
    
    print("\n" + "=" * 60)
    print("📊 ÖZET:")
    print(f"   Program: {'✅ Başarılı' if fixture_success else '❌ Başarısız'}")
    print(f"   Sonuç: {'✅ Başarılı' if result_success else '❌ Başarısız'}")
    print("=" * 60)
